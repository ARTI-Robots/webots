...
vec4 textureSmoothPacked(sampler2D tex, vec2 p, int bias) {
  ...
  float dCenter = texelFetch(tex, pixelCoord, bias).a;
  float dUp = texelFetch(tex, pixelCoord + ivec2(0, 1), bias).a;
  float dDown = texelFetch(tex, pixelCoord + ivec2(0, -1), bias).a;
  float dLeft = texelFetch(tex, pixelCoord + ivec2(-1, 0), bias).a;
  float dRight = texelFetch(tex, pixelCoord + ivec2(1, 0), bias).a;
  float dUpLeft = texelFetch(tex, pixelCoord + ivec2(-1, 1), bias).a;
  float dDownRight = texelFetch(tex, pixelCoord + ivec2(1, -1), bias).a;
  float dDownLeft = texelFetch(tex, pixelCoord + ivec2(-1, -1), bias).a;
  float dUpRight = texelFetch(tex, pixelCoord + ivec2(1, 1), bias).a;
  ...
  if (texSize.x != 1 && texSize.y != 1) {
    ...
    if (isinf(c00.a) || isinf(c01.a) || isinf(c10.a) || isinf(c11.a))
      return c00;
    else {
...
void main() {
  ...
  if (face == FRONT)
    fragColor = textureSmoothPacked(inputTextures[0], faceCoord, 0);
  else if (face == RIGHT)
    fragColor = textureSmoothPacked(inputTextures[1], faceCoord, 0);
  else if (face == BACK)
    fragColor = textureSmoothPacked(inputTextures[2], faceCoord, 0);
  else if (face == LEFT)
    fragColor = textureSmoothPacked(inputTextures[3], faceCoord, 0);
  else if (face == UP)
    fragColor = textureSmoothPacked(inputTextures[4], faceCoord, 0);
  else if (face == DOWN)
    fragColor = textureSmoothPacked(inputTextures[5], faceCoord, 0);

  // The packed alpha channel was needed only to guide
  // interpolation. The final panorama is an RGB image.
  if (!rangeCamera)
